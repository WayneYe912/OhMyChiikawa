...
OhMyChiikawa 是一个轻量的桌面宠物，支持 macOS 和 Windows。启动后，默认角色 **乌萨奇** 会出现在桌面上，你也可以在右键菜单里随时切换到 **吉伊（Chiikawa）**。两个角色都可以拖动、点击互动、跟随鼠标、偶尔自己走动，也会在待机时眨眼和轻微摆动耳朵。
...
- **macOS**: DMG installer provided (arm64 / x64). Download from the Releases page. Supports `chiikawa` CLI launcher.
- **Windows**: NSIS installer and portable edition provided (x64). Supports `chiikawa` CLI launcher (run `install-cli.bat` from the installation directory to add it to PATH).
- **Build commands**:
  - macOS: `npm run dist:mac`
